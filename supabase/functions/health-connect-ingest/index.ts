...
Deno.serve(async(req:Request)=>{
  ...
  for(const a of activities){
    if(!a?.external_id||!a?.activity_date||!a?.activity_type) continue;
  ...
  await supabase.from("sync_sources").upsert({user_id:userId,source:"health_connect",status:"connected",last_sync_at:new Date().toISOString(),details:{sleep:sleepCount,activities:activityCount}},{onConflict:"user_id,source"});
  return respond({ok:true,sleep:sleepCount,activities:activityCount});
});