...
function fitTrainingEffect(...values){
 for(const value of values){let x=num(value);if(x==null)continue;if(x>10&&x<=100)x/=10;if(x>=0&&x<=5.9)return +x.toFixed(1)}
 return null;
}
export function buildBaseFitAnalysis(records=[],session={},configuredMax=null){
 const pts=(Array.isArray(records)?records:[]).map(r=>{
   ...
   const strideRaw=num(r.step_length??r.stride_length),strideLengthM=strideRaw==null?null:(strideRaw>0&&strideRaw<.01?strideRaw*1000:strideRaw),powerW=num(r.enhanced_power??r.power);
   return{t:time,speed,distanceKm,hr:num(r.heart_rate),lat,lon,altitudeKm:num(r.enhanced_altitude??r.altitude),cadenceSpm,strideLengthM,powerW};
