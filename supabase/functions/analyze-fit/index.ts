...
import { makeStrengthReport, normalizeStrengthSets } from "./analysis/strength.js";
...
const handler={async fetch(req:Request){
 ...
 const token=auth.replace(/^Bearer\s+/i,''),{data:userData,error:userError}=await supabase.auth.getUser(token);if(userError||!userData.user)return json({error:'Unauthorized'},401);const userId=userData.user.id;
 let body:any;try{body=await req.json()}catch{return json({error:'Invalid JSON'},400)}const fitFileId=body?.fit_file_id;if(!fitFileId)return json({error:'fit_file_id is required'},400);
 const {data:fitRow,error:fitError}=await supabase.from('fit_files').select('id,user_id,activity_id,storage_path,original_name').eq('id',fitFileId).single();if(fitError||!fitRow)return json({error:'FIT record not found'},404);if(fitRow.user_id!==userId)return json({error:'Forbidden'},403);if(!fitRow.activity_id||!fitRow.storage_path)return json({error:'FIT record is incomplete'},400);if(!fitRow.storage_path.startsWith(userId+'/')||fitRow.storage_path.split('/').some((p:string)=>p==='..'||p==='.'))return json({error:'Invalid FIT path'},403);
 const {data:activityRow,error:activityError}=await supabase.from('activities').select('id,activity_type').eq('id',fitRow.activity_id).eq('user_id',userId).single();if(activityError||!activityRow)return json({error:'Activity record not found'},404);const activityType=String(activityRow.activity_type||'football');
 await supabase.from('fit_files').update({parse_status:'processing',parse_error:null}).eq('id',fitFileId);
 try{
   const {data:blob,error:dlError}=await supabase.storage.from('fit-files').download(fitRow.storage_path);if(dlError||!blob)throw new Error(dlError?.message||'Could not download FIT');const bytes=await blob.arrayBuffer();if(bytes.byteLength>20*1024*1024)throw new Error('FIT exceeds 20 MB limit');if(bytes.byteLength<12||new TextDecoder().decode(new Uint8Array(bytes,8,4))!=='.FIT')throw new Error('Invalid FIT header');
   ...
   const profileRes=await supabase.from('profiles').select('hr_max_bpm').eq('user_id',userId).maybeSingle(),configuredMax=num(profileRes.data?.hr_max_bpm),base=buildBaseFitAnalysis(records,session,configuredMax),{pts,hrZones,speedZones,trackPoints}=base;
   const totalReps=strengthSets.reduce((sum:number,x:any)=>sum+(x.reps||0),0),totalVolumeKg=strengthSets.reduce((sum:number,x:any)=>sum+((x.reps||0)*(x.weight_kg||0)),0),exerciseNames=[...new Set(strengthSets.map((x:any)=>x.exercise).filter(Boolean))].slice(0,20);
