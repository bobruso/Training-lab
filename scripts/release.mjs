...
const files=['app.js','domain.js','runtime.js','styles.css','vendor/supabase.js','vendor/fit-local.js','manifest.webmanifest','privacy.html'];
const app=readFileSync('app.js','utf8').replaceAll('\r\n','\n').replace(/(\.\/(?:domain\.js|vendor\/(?:supabase|fit-local)\.js))\?v=[\w.-]+/g,'$1');
...
for(const file of files)hash.update(file==='app.js'?app:readFileSync(file,'utf8').replaceAll('\r\n','\n'));
hash.update(readFileSync('scripts/sw-template.js','utf8').replaceAll('\r\n','\n'));
const id=hash.digest('hex').slice(0,10);
writeFileSync('app.js',app.replace(/(\.\/(?:domain\.js|vendor\/(?:supabase|fit-local)\.js))(?=['"])/g,`$1?v=${id}`));
