...
function edge({path='owner/test.fit',type='football',blob=fixture()}={}){
...
test('real synthetic FIT binary parses through the full handler with finite metrics',async()=>{
 const {handler,writes}=edge();const response=await handler(req());const data=await response.json();assert.equal(response.status,200,JSON.stringify(data));
 assert.equal(data.summary.distanceKm,6);assert.equal(data.summary.durationSec,1200);assert.equal(data.summary.avgHr,150);assert.equal(data.summary.first10MinM,3000);assert.equal(data.summary.last10MinM,3000);assert.equal(data.summary.absoluteSprintCount,1);
 assert.ok(writes.some(x=>x.table==='activity_analysis'));assert.ok(writes.some(x=>x.row?.parse_status==='parsed'));
});
test('FIT rejects another user storage path and corrupt bytes',async()=>{
 assert.equal((await edge({path:'another/file.fit'}).handler(req())).status,403);
 assert.equal((await edge({blob:Buffer.from('not a FIT')}).handler(req())).status,422);
});
test('FIT strength without set messages does not invent sets',async()=>{
