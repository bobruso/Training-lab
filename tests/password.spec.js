...
test('password login persists in WebView, updates password and never sends OTP',async({page})=>{
 ...
 await page.goto('/');await page.waitForFunction(()=>window.TrainingLab?.state.appReady);await openSettings(page);
 ...
 await page.reload();await page.waitForFunction(()=>window.TrainingLab?.state.appReady);await openSettings(page);await expect(page.locator('#logoutBtn')).toBeVisible();
