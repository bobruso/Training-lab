...
function enhanceActivityIcons(){document.querySelectorAll('.activity-card,.activity-detail-header').forEach(card=>{const text=card.textContent||'',icon=card.querySelector('.activity-icon');if(!icon)return;for(const[label,emoji]of Object.entries(ICONS))if(text.includes(label)){if(icon.textContent!==emoji)icon.textContent=emoji;break}})}
